...
├── config/
│   ├── services_windows.yml        # Configuración de servidores y servicios
│   └── azure_auth.yml              # Credenciales Azure AD (SENSIBLE)
├── static/
...
└── playbooks/
    └── windows/
        ├── restart_services.yml    # Playbook de reinicio
        └── check_services.yml      # Playbook de consulta de estado
```
...
### 3.1 Archivo de servicios (services_windows.yml)
...
grupos:
  - id: grupo_id
    ...
    icono: "🔄"
    nombre_operacion: "Reiniciar Grupo"
    servers:
...
5. Verificar conectividad:
   ```bash
   ansible SERVIDOR -i inventories/prod/hosts -m ansible.windows.win_ping
   ```

### 3.3 Agregar nuevo grupo/tab

Editar `services_windows.yml` y agregar un nuevo bloque bajo `grupos:`:
...
|------------|-----------|
| Usuario WinRM | `/opt/ansible/inventories/prod/group_vars/windows.yml` |
| Azure AD Client Secret | `/opt/rebootwebapp/config/azure_auth.yml` |
...
ansible-playbook playbooks/windows/restart_services.yml \
  -i inventories/prod/hosts \
  -e "target_host=SERVIDOR" \
...
sudo cp nuevo_certificado.crt /etc/nginx/ssl/certificate.crt
sudo systemctl reload nginx
```
...
4. Actualizar `/opt/rebootwebapp/config/azure_auth.yml`
